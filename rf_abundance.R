...
save(inTrain,file = 'inTrain_abundance.Rdata')
trainx <- x[inTrain, ]
testx <- x[-inTrain, ]
...
save(rfFuncs,file = 'rfFuncs_abundance.Rdata')
...
save(optvar,file = 'optvar51_abundance.Rdata')
...
library(ggplot2)
load(file = 'optvar51_abundance.Rdata')
...
x <- dat3[,optvar]
logBBB <- dat2$toxin_abundance
load(file ="inTrain_abundance.Rdata")
trainx <- x[inTrain, ]
testx <- x[-inTrain, ]
trainy <- logBBB[inTrain]
testy <- logBBB[-inTrain]
...
print(rf_gridsearch)
save(rf_gridsearch,file = 'rf_gridsearch_abundance.Rdata')
...
mapset <- read.csv(file = 'map_abundance.csv')
