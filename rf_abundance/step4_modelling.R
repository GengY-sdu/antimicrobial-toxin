...
load(file = "antimicrobial_toxin/rf_abundance/rf_gridsearch_abundance.Rdata")
testset <- testx
...
mapset <- read.csv(file = 'antimicrobial_toxin/rf_abundance/map_abundance.csv')
...
write.csv(predictsum,file = 'antimicrobial_toxin/rf_abundance/final_abundance.csv')

var6 <- varImp(rfmodel6, scale=FALSE)$importance  
var6$pRF <- var6$Overall / sum(var6$Overall) * 100
var6$rfround <- round(var6$pRF,2)
write.csv(var6,file = 'antimicrobial_toxin/rf_abundance/importance_abundance.csv')
