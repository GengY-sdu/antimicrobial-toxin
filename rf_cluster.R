...
save(inTrain,file = 'inTrain_cluster.Rdata')
trainx <- x[inTrain, ]
testx <- x[-inTrain, ]
...
save(rfFuncs,file = 'rfFuncs_cluster.Rdata')
...
save(optvar,file = 'optvar51_cluster.Rdata')
...
#step2:hyperparamter tuning#####################################################

rm(list = ls())
library(caret)
library(randomForest)
library(ggplot2)
load(file = 'optvar51_cluster.Rdata')
...
x <- dat3[,optvar]
logBBB <- dat2$toxin_clusters
load(file ="inTrain_cluster.Rdata")
trainx <- x[inTrain, ]
testx <- x[-inTrain, ]
trainy <- logBBB[inTrain]
testy <- logBBB[-inTrain]
...
tunegrid <- expand.grid(mtry=c(1,4,8,10,12,16,20,24,28,30,32,36,40),
...
print(rf_gridsearch)
save(rf_gridsearch,file = 'rf_gridsearch_cluster.Rdata')
...
ggplot(data = plot, aes(x = rf_gridsearch.pred.obs, y = rf_gridsearch.pred.pred)) + 
  ...
  labs(x = "Observed antimicrobial toxin clusters", y = "Predicted antimicrobial toxin clusters") +
  theme_classic() +
  theme(axis.text=element_text(size=12,color = 'black'),axis.title=element_text(size=14,color = 'black'),
        axis.line.x=element_line(linetype=1,color="black",size=0),
        axis.line.y=element_line(linetype=1,color="black",size=0),
...
mapset <- read.csv(file = 'map_cluster.csv')
