...
save(inTrain,file = 'inTrain_family.Rdata')
trainx <- x[inTrain, ]
testx <- x[-inTrain, ]
...
save(rfFuncs,file = 'rfFuncs_family.Rdata')
...
save(optvar,file = 'optvar51_family.Rdata')
...
library(ggplot2)
load(file = 'optvar51_family.Rdata')
...
x <- dat3[,optvar]
logBBB <- dat2$toxin_families
load(file ="inTrain_family.Rdata")
trainx <- x[inTrain, ]
testx <- x[-inTrain, ]
trainy <- logBBB[inTrain]
testy <- logBBB[-inTrain]
...
tunegrid <- expand.grid(mtry=c(1,3,5,8,10,13,15,18,20,23,25,28,30),
...
print(rf_gridsearch)
save(rf_gridsearch,file = 'rf_gridsearch_family.Rdata')
...
mapset <- read.csv(file = 'map_family.csv')
