...
mapset <- read.csv(file = 'antimicrobial_toxin/rf_family/map_family.csv')
...
write.csv(predictsum,file = 'antimicrobial_toxin/rf_family/final_family.csv')

var6 <- varImp(rfmodel6, scale=FALSE)$importance  
var6$pRF <- var6$Overall / sum(var6$Overall) * 100
var6$rfround <- round(var6$pRF,2)
write.csv(var6,file = 'antimicrobial_toxin/rf_family/importance_family.csv')
